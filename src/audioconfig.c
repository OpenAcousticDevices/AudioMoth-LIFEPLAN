...
#define MAXIMUM_NUMBER_OF_BYTES             15
#define RECEIVE_BUFFER_SIZE_IN_BYTES        15
...
#define NUMBER_OF_START_STOP_PERIODS        4
...
enum state_t {IDLE, START_BITS, DATA_BITS, DATA_OR_STOP_BITS};

enum receivedBit_t {HIGH_BIT, LOW_BIT};
...
void AudioConfig_handleAudioConfiguration() {
    ...
    AudioMoth_enableMicrophone(AM_NORMAL_GAIN_RANGE, CONFIG_GAIN, CONFIG_CLOCK_DIVIDER, CONFIG_ACQUISITION_CYCLES, CONFIG_OVERSAMPLE_RATE);
    ...
    while (!cancel) {
        ...
        if (configSampleReady) {
            ...
            float sample = Butterworth_applyBandPassFilter((float)configSample, &carrierFilter, &carrierFilterCoefficients);

            float agcOutput = Butterworth_applyLowPassFilter(sample > 0 ? sample : -sample, &agcFilter, &agcFilterCoefficients);

            if (agcOutput != 0) sample /= agcOutput;

            /* Demodulate input sound */

            uint8_t index = (uint32_t)omegaT & (SINE_TABLE_LENGTH - 1);

            float channel1 = Biquad_applyFilter(sineTable[index] * sample, &channel1Filter, &channelFilterCoefficients);

            index -= SINE_QUARTER_CYCLE_INCREMENT;

            float channel2 = Biquad_applyFilter(sineTable[index] * sample, &channel2Filter, &channelFilterCoefficients);
            ...
            if ((lastValue > 0 && channel1 < 0) || (lastValue < 0 && channel1 > 0)) {
                ...
                if (state == IDLE) {
                    ...
                    if (IN_PERIOD(period, START_STOP_BIT_PERIOD, PERIOD_TOLERANCE)) {
                        ...
                        if (bitCount > NUMBER_OF_START_STOP_PERIODS) {
                ...
                if (state == DATA_BITS || state == DATA_OR_STOP_BITS) {
                    ...
                    if (period > MIN_BIT_PERIOD && period < MAX_BIT_PERIOD) {
                        ...
                        if (bitCount == NUMBER_OF_START_STOP_PERIODS && state == DATA_OR_STOP_BITS && byteCount > CRC_SIZE_IN_BYTES) {
